...
export const Badge: React.FC<BadgeProps> = ({ text, variant = 'default' }) => {
  const cls = `inline-block px-2 py-0.5 rounded text-xs font-medium ${badgeColors[variant]}`;
...
export const ScoreDisplay: React.FC<ScoreDisplayProps> = ({
  ...
  const parts = [
    React.createElement('span', { className: 'text-sm text-gray-400', key: 'label' }, label),
    React.createElement(
      'span',
      { className: 'text-3xl font-bold text-indigo-400', key: 'score' },
      score.toLocaleString()
    ),
  ];

  if (kills !== undefined || survivalTime !== undefined) {
    const subParts: React.ReactNode[] = [];
    if (kills !== undefined) {
      subParts.push(React.createElement('span', { key: 'kills' }, `🗡️ ${kills} kills`));
    }
    if (survivalTime !== undefined) {
      subParts.push(React.createElement('span', { key: 'time' }, `⏱️ ${formatTime(survivalTime)}`));
    }
    parts.push(
      React.createElement(
        'div',
        { className: 'flex gap-3 text-sm text-gray-400', key: 'sub' },
        ...subParts
      )
    );
  }

  return React.createElement('div', { className: 'flex flex-col items-center gap-1' }, ...parts);
};
